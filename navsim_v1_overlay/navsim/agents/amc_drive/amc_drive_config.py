...
class AMCDriveConfig:
    ...
    # consequence-aware lateral de-redundancy (R22-R28). dedup_weight=0 disables.
    dedup_weight: float = 0.0
    dedup_tau_arc: float = 0.5   # optimal-band gate: arc >= this quantile of scene max
    dedup_centerline_cache: str = ""  # path to r28 centerline cache npz (empty = disabled)
    dedup_lateral_reg: float = 0.0   # lateral quadratic regularizer (0 disables; 0.4 => ~5m spread)
